...
//        }
        ...
        if(!existingUser.isPresent() || isDeletedUser) {
            ...
            profile_path = userAuthUtils.saveProfileImage(dto.getMultipartFile(), user);
    ...
    public UserListResponseDto getAllUsers() {
        ...
        Optional<Role> role = roleRepository.findByRole(com.user.auth.constants.Role.USER.name());
    ...
    public void deleteUserById(Long userId) throws Exception {
        ...
        if (loggedInUser.getRoles().stream().noneMatch(x -> x.getRole().equalsIgnoreCase(com.user.auth.constants.Role.ADMIN.name()))) {
